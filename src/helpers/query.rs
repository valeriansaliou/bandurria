...
use super::{authentication, normalize, time};
...
pub async fn find_author_id(
    ...
    let author = sqlx::query("SELECT id, trusted FROM authors WHERE email_hash = ?")
        ...
        .map(|author| {
            let mut trusted = author.get("trusted");

            // Forcibly trust user? (if administrator)
            if trusted == false && authentication::check_email_is_admin(email) {
                trusted = true;
            }
...
pub async fn find_or_create_author_id(
    ...
    match find_author_id(db, email).await? {
        ...
        None => {
            ...
            // Check if user can be auto-trusted (if administrator)
            let trusted = authentication::check_email_is_admin(email);
...
pub async fn resolve_comment_page_id(
    ...
    let comment_page_id = sqlx::query("SELECT page_id FROM comments WHERE id = ? AND approved = 1")
        ...
        .await
        .map_err(|err| {
            error!("Failed loading comment: {}, because: {}", comment_id, err);

            Status::InternalServerError
        })?
        .map(|page| page.get("page_id"));
...
pub async fn insert_comment_for_page_id_and_author_id(
    ...
    sqlx::query(
        r#"INSERT INTO comments (
            id, text, approved, created_at, author_id, page_id, reply_to_id
        )
            VALUES (?, ?, ?, ?, ?, ?, ?)"#,
    )
    .bind(&comment_id)
