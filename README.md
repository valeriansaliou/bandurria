...
Bandurria can optionally show user avatars next to their names, using the 3rd party [Gravatar](https://gravatar.com/) service. Avatars are fetched from Gravatar and cached in your MySQL database as BLOBs. They are then periodically refreshed when accessed. Avatars are limited to a maximum size of `64KB`, so make sure not to configure Bandurria to request large avatar sizes (in pixels).
...
```bash
docker pull valeriansaliou/bandurria:v1.1.0
```
...
```bash
docker run -p 8080:8080 -v /path/to/your/bandurria/config.cfg:/etc/bandurria.cfg valeriansaliou/bandurria:v1.1.0
```
