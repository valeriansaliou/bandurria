...
Spam is prevented by requiring user browsers to submit the result to a Proof of Work challenge (based on an improved variant of [Hashcash](http://www.hashcash.org/papers/hashcash.pdf) in order to [reduce the mint time variance](https://friendlycaptcha.com/insights/controlling-variance-in-proof-of-work-algorithms/)), minted when the user submits their comment. This spam prevention method is CAPTCHA-free and hassle-free, as it should only take a few seconds to compute under normal circumstances.
